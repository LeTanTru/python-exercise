...
def levenshtein_distance(s1, s2):
    ...
    if s1[-1] == s2[-1]:
        cost = 0
        distance, _ = levenshtein_distance(s1[:-1], s2[:-1])
        return distance, []  
    else:
        cost = 1
        distance_insert, _ = levenshtein_distance(s1, s2[:-1])  
        distance_delete, _ = levenshtein_distance(s1[:-1], s2)  
        distance_replace, _ = levenshtein_distance(s1[:-1], s2[:-1]) 
...
def spell_check(word, dictionary):
    ...
    for correct_word in dictionary:
        distance, _ = levenshtein_distance(correct_word, word)
