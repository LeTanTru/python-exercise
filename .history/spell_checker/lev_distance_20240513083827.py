...
def levenshtein_distance(a, s2):
    ...
    if a[-1] == s2[-1]:
        return levenshtein_distance(a[:-1], s2[:-1])
    else:
        cost = 1
        distance_insert = levenshtein_distance(a, s2[:-1])  
        distance_delete = levenshtein_distance(a[:-1], s2)  
        distance_replace = levenshtein_distance(a[:-1], s2[:-1]) 
