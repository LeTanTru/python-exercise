...
def wagner_fischer(s1, s2):
    ...
    return current_row[len_s1], current_row

def spell_check(word, dictionary):
    suggestions = []

    for correct_word in dictionary:
        distance, cur = wagner_fischer(word, correct_word)
        suggestions.append((correct_word, distance, cur))
...
for word, distance, cur in suggestions:
    print(f"{word} (Distance: {distance}) cur: {cur}")
